...
static void * Activate_API_Thread_Func(void * arg)
{
	...
	if(std_broadcast_data.activation == 1)	/* 激活成功 */
	{
...
static void DJI_Pro_Control_Management_CallBack(ProHeader *header)
{
	...
	if(std_broadcast_data.obtained_control == 1)	/* 获取控制权成功 */
	{
...
/* write by zhouzibo in 2015-11-23 */
int DJI_Pro_Get_Pos(api_pos_data_t *p_user_buf)
{	
	pthread_mutex_lock(&std_msg_lock);
	*p_user_buf = std_broadcast_data.pos;
	pthread_mutex_unlock(&std_msg_lock);
	return 0;
}
...
#if 0
	printf("longti:%lf, lati:%lf, alti:%lf\n", std_broadcast_data.pos.longti, std_broadcast_data.pos.lati, std_broadcast_data.pos.alti);
#endif
}
...
void init_aircraft_status(void)
{
	aircraft_status.activation = -1;
	aircraft_status.obtained_control = -1;
}