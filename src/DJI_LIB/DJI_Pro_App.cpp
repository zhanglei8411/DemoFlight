        ...
        else
        {
            if(ack_data == SDK_ACTIVATE_SUCCESS)
            {
                printf("Activation Successfully\n");
					 ...
					 aircraft_status.activation = std_broadcast_data.activation;
...
static void DJI_Pro_Control_Management_CallBack(ProHeader *header)
{
	...
	aircraft_status.obtained_control = std_broadcast_data.obtained_control;
}
...
/* write by zhouzibo in 2015-12-31 */
int XY_Pro_Get_Pos_Height(float *_height)
{	
	pthread_mutex_lock(&std_msg_lock);
	*_height = std_broadcast_data.pos.height;
	pthread_mutex_unlock(&std_msg_lock);
	return 0;
}
