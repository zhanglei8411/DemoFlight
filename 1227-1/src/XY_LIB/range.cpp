...
static void *KS103_Thread_Func(void * arg)
{
	...
	while(1)
	{
		...
		//KS103_Cal_Detect_Result(rbuf, wbuf[2]);
#if 0
		printf("cmd:0x%X, retries:%d, data:%fm\n", wbuf[2], i, KS103_Cal_Detect_Result(rbuf, wbuf[2]) );
#endif

#if 1
		result = KS103_Cal_Detect_Result(rbuf, wbuf[2]);
		pthread_mutex_lock(&ultra_msg_lock);
		XY_Debug_Get_Ultrasonic_Data(&debug_package.ultrasonic_data, result);
